...
- **[2026-08-27] Google yorumları eklenmedi — doğrulanmış gerçek yorum bulunamadı** — Kullanıcı ana sayfaya işletmenin gerçek Google Haritalar 5 yıldızlı yorumlarından 15 tanesinin (isimler gizli) eklenmesini istedi. Google Haritalar JS ile render edildiği için basit fetch ile yorum verisi çekilemedi; Yandex Haritalar'daki aynı işletme profilinde ise "İlk katkıyı siz yapın" yazıyor (sıfır yorum). Gerçek, doğrulanabilir yorum metni bulunamadığı için hiçbir yorum eklenmedi/uydurulmadı — kullanıcıya doğrudan sorulacak (Google İşletme Profili linki veya yorum ekran görüntüsü istenecek).
