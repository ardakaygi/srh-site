...
- **Resim yükleme yerel dosya sistemine yazıyor** (`src/lib/imageUpload.ts`, `public/uploads/`). Bu, kalıcı bir Node.js sunucusunda çalışır ama salt-okunur dosya sistemine sahip serverless platformlarda (ör. Vercel'in varsayılan çalışma zamanı) çalışmaz — üretimde gerçek bir nesne depolama (S3 uyumlu) katmanına geçilmesi gerekebilir. Ayrıca `public/uploads/` git'e dahil değil (`.gitignore`), yani deploy/host değişince kalıcı bir volume/disk planlanmalı.
- **"Gerçek bir CMS gibi her metin/görsel" hedefi genişletildi ama tam değil.** Servis talepleri, markalar (+logo yükleme), iller (+il fotoğrafı yükleme), modeller, blog (+kapak görseli yükleme) ve SSS artık tamamen admin panelden yönetiliyor (Prisma tablolarına taşındı). Ana sayfa hero/CTA metinleri, marka sloganı (header+footer) ve güven rozetleri de `SiteSetting` tablosu üzerinden düzenlenebilir. Marka logoları artık "öne çıkan" listesini otomatik belirliyor (hangi markanın `logoUrl`'ü varsa o listelenir — sabit slug listesi yok). Ancak hâlâ koda gömülü kalanlar: 12 kalemlik "Teknik Servis Kapsamımız" servis kategorisi ızgarası (her biri ikon + başlık + açıklama + 3 madde — ikonlar React bileşeni olduğu için basit metin editörüne taşınamıyor), sabit sayfaların düz metni (Kurumsal, Tamir Merkezi, Yasal Uyarı, SSS'nin "Faydalı Bağlantılar" bloğu), ve `site-config.ts`'teki temel işletme bilgileri (telefon/adres/e-posta/garanti). Bu sonuncuyu editable yapmak, `siteConfig`'in senkron import edildiği client component'leri (MobileNav, ServiceRequestForm vb.) yeniden düzenlemeyi gerektiren ayrı, daha büyük bir iş.
